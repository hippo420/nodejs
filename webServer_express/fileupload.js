...
app1.use('/uploads',static(path.join(__dirname,'uploads')));
...
router.route('/action/upload').post(upload.array('file',1),
function(req,res){
    console.log('upload.....');
    let files = req.files;
    console.log('업로드된 파일');
    ...
    res.write('<h1>파일 업로드 내용</h1>');
    res.write('<div><p>원본파일명 :'+originalname+'</p></div>');
    res.write('<div><p>저장된파일명 :'+filename+'</p></div>');
    res.write('<div><p>확장자 :'+mimetype+'</p></div>');
    res.write('<div><p>사이즈 :'+size+'</p></div>');
    res.end();
